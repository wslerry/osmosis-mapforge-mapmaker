...
def start_new_file():
    ...
    print("<?xml version='1.0' encoding='UTF-8'?>", file=open_file)
...
def add_ring_nodes(ring):
    ...
    for count in range(ring.GetPointCount() - 1):
        ...
        print("  <node timestamp='1969-12-31T23:59:59Z' id='%s' version='1' uid='0' user='' lon='%s' lat='%s' />" % (
...
def add_ring_way(ring): 
    ...
    print("  <way timestamp='1969-12-31T23:59:59Z' id='%s' version='1' uid='0' user=''>" % id_counter, file=open_file)
    way_id = id_counter
    id_counter += 1
    count = 0
    for i in ids:
...
# Write out the outer ways in the relation
            for count in range( len(ringways) ):
                ...
                if ids and len(ids) > 1: 
                    # print("<way timestamp='1969-12-31T23:59:59Z' changeset='-1' id='%s' version='1'>" % id_counter, file=open_file)
                    print("  <way timestamp='1969-12-31T23:59:59Z' id='%s' version='1' uid='0' user=''>" % id_counter,
                          file=open_file)
...
#add the inner ways
                ...
                print(
                    "  <relation timestamp='1969-12-31T23:59:59Z' id='%s' version='1' uid='0' user=''><tag k='type' v='multipolygon' />" % id_counter,
                    file=open_file)
                id_counter += 1
